...
const uploadPaymentReceiptControllers = async (req, res) => {
  try {
    ...
    const filesPayload = files.paymentReceipt.map((file) => ({
      url: file?.location,
      date,
      time,
      day,
    ...
    doc.paymentReceipt.push(...filesPayload);

    await doc.save();
    return res
      .status(200)
      .json({ message: "payment receipt updated successfully." });
  } catch (error) {
    console.log(error);
...
module.exports = uploadPaymentReceiptControllers;
